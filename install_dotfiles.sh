...
function __zdi_3_symlink_files() {
	NOCOPY=('.' '..' '.git' '.gitignore' '.editorconfig')
	NOCOPYTYPE=('swp')
	SYMLINKED=0
	...
	for file in "${REPO_PATH}"/.*; do
		...
		TARGET=~/"$FILENAME"
		for to_skip in "${NOCOPY[@]}"; do
			if [ "$FILENAME" == "$to_skip" ]; then
				__zdi_log Skipping "$to_skip"
				continue 2
			fi
		done
		for to_skip in "${NOCOPYTYPE[@]}"; do
			if [ "${FILENAME##*.}" == "$to_skip" ]; then
				__zdi_log Skipping "$to_skip"
				continue
			fi
	...
	for file in "${REPO}"/lib/gitconfig/tool.*.gitconfig; do
		...
		if command -v "$TOOLNAME" &>/dev/null; then
		  TOOLPATH=$(which "$TOOLNAME")
			printf ' %sfound%s in %s, adding %s \n' "${c_green}" "${c_normal}" "$TOOLPATH" "$TOOLBASENAME"
...
function __zdi_6_load_bash_extras() {
	[ -f "$HOME/.bash-git-prompt/gitprompt.sh" ] || install_warn "Git prompt not found. Clone bash-git-prompt repository to .bash-git-prompt"
