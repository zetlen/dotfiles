...
_z_printable_colours=256

# Return a colour that contrasts with the given colour
# Bash only does integer division, so keep it integral
function _z_color_contrast {
...
function _z_color_print_run {
	local i
	for ((i = "$1"; i < "$1" + "$2" && i < _z_printable_colours; i++)); do
		_z_color_print_swatch "$i"
	done
	printf "  "
}

# Print blocks of colours
function _z_color_print_blocks {
	local start="$1" i
	...
	for ((i = start; i <= end; i += (blocks_per_line - 1) * block_length)); do
		...
		for ((row = 0; row < block_rows; row++)); do
			...
			for ((block = 0; block < blocks_per_line; block++)); do
				_z_color_print_run $((i + (block * block_length))) "$block_cols"
			done
